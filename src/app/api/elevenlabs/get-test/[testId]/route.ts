...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `https://api.elevenlabs.io/v1/convai/agent-testing/${testId}`, 
      {
        method: 'GET',
        headers: {
          'xi-api-key': apiKey,
    ...
    if (!response.ok) {
      ...
      let errorMessage = `Failed to fetch test: ${response.statusText}`;
      try {
    ...
    console.log('✅ Test fetched successfully:', {
      ...
      successExamplesCount: testData.success_examples?.length || 0,
      failureExamplesCount: testData.failure_examples?.length || 0
    });
